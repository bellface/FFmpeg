...
wrtcapture_region_wnd_proc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
    PAINTSTRUCT ps;
    HDC hdc;
    RECT rect;
 ...
 * @param wrtcapture wrtcapture context.
 * @return 0 success, !0 failure
 */
static int
wrtcapture_region_wnd_init(AVFormatContext *s1, struct wrtcapture *wrtcapture)
{
    HWND hwnd;
    RECT rect = wrtcapture->clip_rect;
    HRGN region = NULL;
    ...
    SetWindowLongPtr(hwnd, GWLP_WNDPROC, (LONG_PTR) wrtcapture_region_wnd_proc);

    ShowWindow(hwnd, SW_SHOW);

    wrtcapture->region_hwnd = hwnd;
 ...
 */
static void
wrtcapture_region_wnd_destroy(AVFormatContext *s1, struct wrtcapture *wrtcapture)
{
    if (wrtcapture->region_hwnd)
        DestroyWindow(wrtcapture->region_hwnd);
    wrtcapture->region_hwnd = NULL;
}
 ...
 */
static void
wrtcapture_region_wnd_update(AVFormatContext *s1, struct wrtcapture *wrtcapture)
{
    HWND hwnd = wrtcapture->region_hwnd;
    MSG msg;
 ...
 */
static int
wrtcapture_read_header(AVFormatContext *s1)
{
    struct wrtcapture *wrtcapture = s1->priv_data;
    ...
    if (!strncmp(filename, "title=", 6)) {
        ...
        if (wrtcapture->show_region) {
            av_log(s1, AV_LOG_WARNING,
                    "Can't show region when grabbing a window.\n");
            wrtcapture->show_region = 0;
        }
    ...
    if (!wrtcapture->width || !wrtcapture->height) {
        ...
        clip_rect.left = wrtcapture->offset_x;
        clip_rect.top = wrtcapture->offset_y;
        clip_rect.right = wrtcapture->width + wrtcapture->offset_x;
        clip_rect.bottom = wrtcapture->height + wrtcapture->offset_y;
    }
    ...
    wrtcapture->frame_size  = bmp.bmWidthBytes * bmp.bmHeight * bmp.bmPlanes;
    wrtcapture->header_size = sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER) +
                           (bpp <= 8 ? (1 << bpp) : 0) * sizeof(RGBQUAD) /* palette size */;
    wrtcapture->time_base   = av_inv_q(wrtcapture->framerate);
    wrtcapture->time_frame  = av_gettime() / av_q2d(wrtcapture->time_base);

    wrtcapture->hwnd       = hwnd;
    wrtcapture->source_hdc = source_hdc;
    wrtcapture->dest_hdc   = dest_hdc;
    wrtcapture->hbmp       = hbmp;
    wrtcapture->bmi        = bmi;
    wrtcapture->buffer     = buffer;
    wrtcapture->clip_rect  = clip_rect;

    wrtcapture->cursor_error_printed = 0;

    if (wrtcapture->show_region) {
        if (wrtcapture_region_wnd_init(s1, wrtcapture)) {
            ret = AVERROR(EIO);
            goto error;
        }
    }

    st->avg_frame_rate = av_inv_q(wrtcapture->time_base);

    st->codecpar->codec_type = AVMEDIA_TYPE_VIDEO;
    st->codecpar->codec_id   = AV_CODEC_ID_BMP;
    st->codecpar->bit_rate   = (wrtcapture->header_size + wrtcapture->frame_size) * 1/av_q2d(wrtcapture->time_base) * 8;
 ...
 */
static void paint_mouse_pointer(AVFormatContext *s1, struct wrtcapture *wrtcapture)
{
    CURSORINFO ci = {0};

#define CURSOR_ERROR(str)                 \
    if (!wrtcapture->cursor_error_printed) {       \
        WIN32_API_ERROR(str);             \
        wrtcapture->cursor_error_printed = 1;      \
    }
    ...
    if (GetCursorInfo(&ci)) {
        ...
        ICONINFO info;
        POINT pos;
        RECT clip_rect = wrtcapture->clip_rect;
        HWND hwnd = wrtcapture->hwnd;
        int horzres = GetDeviceCaps(wrtcapture->source_hdc, HORZRES);
        int vertres = GetDeviceCaps(wrtcapture->source_hdc, VERTRES);
        int desktophorzres = GetDeviceCaps(wrtcapture->source_hdc, DESKTOPHORZRES);
        int desktopvertres = GetDeviceCaps(wrtcapture->source_hdc, DESKTOPVERTRES);
        info.hbmMask = NULL;
        info.hbmColor = NULL;
        ...
        if (pos.x >= 0 && pos.x <= clip_rect.right - clip_rect.left &&
            ...
            if (!DrawIcon(wrtcapture->dest_hdc, pos.x, pos.y, icon))
 ...
 */
static int wrtcapture_read_packet(AVFormatContext *s1, AVPacket *pkt)
{
    struct wrtcapture *wrtcapture = s1->priv_data;

    HDC        dest_hdc   = wrtcapture->dest_hdc;
    HDC        source_hdc = wrtcapture->source_hdc;
    RECT       clip_rect  = wrtcapture->clip_rect;
    AVRational time_base  = wrtcapture->time_base;
    int64_t    time_frame = wrtcapture->time_frame;

    BITMAPFILEHEADER bfh;
    int file_size = wrtcapture->header_size + wrtcapture->frame_size;
    ...
    if (wrtcapture->show_region)
        wrtcapture_region_wnd_update(s1, wrtcapture);
    ...
    if (wrtcapture->draw_mouse)
        paint_mouse_pointer(s1, wrtcapture);
    ...
    bfh.bfReserved1 = 0;
    bfh.bfReserved2 = 0;
    bfh.bfOffBits = wrtcapture->header_size;

    memcpy(pkt->data, &bfh, sizeof(bfh));

    memcpy(pkt->data + sizeof(bfh), &wrtcapture->bmi.bmiHeader, sizeof(wrtcapture->bmi.bmiHeader));

    if (wrtcapture->bmi.bmiHeader.biBitCount <= 8)
        GetDIBColorTable(dest_hdc, 0, 1 << wrtcapture->bmi.bmiHeader.biBitCount,
                (RGBQUAD *) (pkt->data + sizeof(bfh) + sizeof(wrtcapture->bmi.bmiHeader)));

    memcpy(pkt->data + wrtcapture->header_size, wrtcapture->buffer, wrtcapture->frame_size);

    wrtcapture->time_frame = time_frame;

    return wrtcapture->header_size + wrtcapture->frame_size;
}
 ...
 * @return 0 success, !0 failure
 */
static int wrtcapture_read_close(AVFormatContext *s1)
{
    struct wrtcapture *s = s1->priv_data;

    if (s->show_region)
        wrtcapture_region_wnd_destroy(s1, s);
...
#define OFFSET(x) offsetof(struct wrtcapture, x)
...
static const AVClass wrtcapture_class = {
    .class_name = "Wrtcapture indev",
...
AVInputFormat ff_wrtcapture_demuxer = {
    .name           = "wrtcapture",
    .long_name      = NULL_IF_CONFIG_SMALL("GDI API Windows frame grabber"),
    .priv_data_size = sizeof(struct wrtcapture),
    .read_header    = wrtcapture_read_header,
    .read_packet    = wrtcapture_read_packet,
    .read_close     = wrtcapture_read_close,
    .flags          = AVFMT_NOFILE,
    .priv_class     = &wrtcapture_class,
};
